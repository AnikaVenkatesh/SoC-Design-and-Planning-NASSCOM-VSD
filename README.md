...
5. *Die*
 Together, the core and the surrounding pad ring constitute the die, which is the fundamental unit manufactured in semiconductor fabrication. Dies are replicated across a silicon wafer, and each die functions as one complete chip after packaging. The die layout must follow strict design rules to ensure manufacturability, reliability, and yield.
